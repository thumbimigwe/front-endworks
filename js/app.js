...
jQuery(document).ready(function) {
	var intro = $('.hero'),
		projectsContainer = $('.contentsWrapper'),
		projectsSlider = projectsContainer.children('.contents'),
		singleProjectContent = $('.cd-project-content'),
		sliderNav = $('.contents-navigation');
	...
	//show the projects slider if user clicks the showBody button
	intro.on('click', 'a[data-action="showBody"]', function(event) {
	...
	intro.on('click', function(event) {
		...
		if( intro.hasClass('projects-visible') && !$(event.target).is('a[data-action="showBody"]') ) {
	...
	projectsContainer.on('click', '.contents a', function(event) {
		var mq = checkMQ();
		event.preventDefault();
	...
	function checkMQ() {
		...
		return window.getComputedStyle(document.querySelector('.contentsWrapper'), '::before').getPropertyValue('content').replace(/'/g, "").replace(/"/g, "");
	}
